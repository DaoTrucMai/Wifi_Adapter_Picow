...
static void bus_reset(void) {
    ...
    // Reinitialize endpoints on bus reset to recover cleanly from disconnects,
    // host-initiated reset, or transient transfer state.
    config_descriptor.wTotalLength = USB_CONFIG_TOTAL_LEN_BASE;
    setup_endpoints();

    struct usb_endpoint_configuration* eps = dev_config.endpoints;
    for (int i = 0; i < USB_NUM_ENDPOINTS; i++) {
        struct usb_endpoint_configuration* ep = &eps[i];
        if (!ep->descriptor) continue;
        ep->next_pid = 0;
        ep->length = 0;
        ep->pos = 0;
        ep->pos_send = 0;
...
static volatile uint8_t *get_dpram_buffer(struct usb_endpoint_configuration *ep) {
    ...
    uint pre = dpram_alloc;
    dpram_alloc += ep->descriptor->wMaxPacketSize >> 6;
    if (ep->double_buffer) dpram_alloc += ep->descriptor->wMaxPacketSize >> 6;
...
static void setup_endpoint(struct usb_endpoint_configuration *ep) {
    ...
    // Clear any stale buffer-control state from a previous configuration/bus
    // reset so we don't accidentally complete "old" buffers and feed garbage
    // into handlers.
    if (ep->buffer_control) *ep->buffer_control = 0;
    ...
    if (!is_ep0(ep)) {
        // EP0 is implicit and not included in the configuration descriptor.
...
static void handle_ep_buff_done(struct usb_endpoint_configuration *ep) {
    ...
    /*
     * Bulk OUT is a byte stream. For streaming OUT endpoints (UNKNOWN_SIZE),
     * short packets are not "message boundaries" and should not complete the
     * transfer; we want to keep the endpoint continuously armed.
     */
    streaming_out = (!ep_is_tx(ep) && ep->length == UNKNOWN_SIZE);

    /*
     * Bulk IN: if we end a transfer on an exact max-packet boundary, the host
     * may keep the read URB pending (waiting for a short packet). Emit a ZLP
     * to terminate the transfer cleanly.
     */
    if (ep_is_tx(ep) && ep->length != UNKNOWN_SIZE && ep->length > 0 &&
        (ep->length % ep->descriptor->wMaxPacketSize) == 0) {
        need_zlp = true;
    }

    // If we just sent the final max-packet of a transfer that needs a ZLP,
    // don't complete yet; arm a 0-length packet instead.
    if (need_zlp && len == ep->descriptor->wMaxPacketSize && ep->pos == ep->length) {
        start_data_packet(ep);  // emits ZLP (len==0) because remaining==0
        return;
    }

    if ((!streaming_out && len < ep->descriptor->wMaxPacketSize) ||
        (len == ep->descriptor->wMaxPacketSize && ep->length == ep->pos) ||
        ...
        // Continue transfers for both buffered and streaming endpoints.
        // For streaming endpoints (data_buffer==NULL), the handler is called per
        // packet to fill/consume DPRAM. The next packet must be armed here.
...
void usb_init_transfer(struct usb_endpoint_configuration *ep, int32_t len) {
    ...
    ep->is_completed = false;
    ep->status = STATUS_BUSY;
    // Ensure buffer control does not retain AVAIL/FULL bits from a previous
    // transfer (especially important for double-buffered streaming OUT).
    if (ep->buffer_control) *ep->buffer_control = 0;
    usb_hw_clear->buf_status = ep->bit;
    start_data_packet(ep);
}
...
bool usb_is_transfer_completed(struct usb_endpoint_configuration *ep) { return ep->is_completed; }
