...
static void pico_tx_complete(struct urb *urb)
{
    ...
    if (pdev->last_tx_is_dhcp && pdev->dhcp_tx_complete_dbg_count < 10)
    {
                             ...
                             const char *psk, u8 psk_len)
{
    ...
    h.hdr_len = PWU_HDR_LEN;
    h.seq = cpu_to_le16(pdev->last_seq++);
    h.payload_len = cpu_to_le16(2 + ssid_len + psk_len);
    h.xid = cpu_to_le32(0);
    ...
    p[0] = ssid_len;
    p[1] = psk_len;
    memcpy(p + 2, ssid, ssid_len);
    if (psk && psk_len)
        memcpy(p + 2 + ssid_len, psk, psk_len);

    ret = pico_tx_submit(pdev, msg, PWU_HDR_LEN + 2 + ssid_len + psk_len);
    if (ret == 0)
    {
...
static void pico_handle_data_rx(struct pico_dev *pdev, const u8 *payload, size_t plen)
{
    ...
    if (is_dhcp && pdev->dhcp_rx_dbg_count < 10)
    {
        dev_info(&pdev->intf->dev,
    ...
    if (is_dhcp)
    {
        // For bring-up: avoid checksum-related drops in UDP receive path.
...
static netdev_tx_t pico_ndo_start_xmit(struct sk_buff *skb, struct net_device *ndev)
{
    ...
    if (is_dhcp && pdev->dhcp_tx_dbg_count < 10)
    {
